...
export default function EditEmployee() {
    ...
    const getCurrentUser = async () => {
        console.log(state);
        try {
            const res = await axios.get(`${admin_api}/get-user-by-id?id=${state}`);
            console.log(res.data);
            if (res.data.status) {
                setCurrentUser(res.data.user) // {status,message,user:{email,user:{}}}
                return;
            }
            else {
    ...
    return (
        <div className='container vh-100'>
            <h2 className='my-3 text-center'>Edit Employee Page</h2>
